...
public class SqlLiteDownloadProvider implements DownloadProvider {
    ...
    private SqlLiteDownloadProvider(DownloadManager manager) {
        ...
        File dbFile = new File(manager.getConfig().getDatabasePath(), "download.db");
        if (dbFile.exists()) {
    ...
    public void updateDownloadTask(DownloadTask task) {
        ContentValues values = createDownloadTaskValues(task);
        db.update(DOWNLOAD_TABLE, values, DownloadTask.URL + "=?", new String[]{task.getUrl()});
    ...
    public DownloadTask findDownloadTaskByUrl(String url) {
        ...
        Cursor cursor = db.query(DOWNLOAD_TABLE, null, DownloadTask.URL + "=?", new String[]{url}, null, null, null);
    ...
    public void deleteOldData(String url) {
        db.delete(DOWNLOAD_TABLE, DownloadTask.URL + " = ?", new String[]{
                url
        });
    }
    ...
    private ContentValues createDownloadTaskValues(DownloadTask task) {
        ...
        values.put(DownloadTask.ID, task.getTaskId());
        ...
        values.put(DownloadTask.NAME, task.getFileName());
    ...
    private DownloadTask restoreDownloadTaskFromCursor(Cursor cursor) {
        ...
        task.setTaskId(cursor.getString(cursor.getColumnIndex(DownloadTask.ID)));
        task.setFileName(cursor.getString(cursor.getColumnIndex(DownloadTask.NAME)));
